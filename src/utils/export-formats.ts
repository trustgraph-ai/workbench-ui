...
export function exportToCSV(taxonomy: Taxonomy): string {
  ...
  concepts.forEach(concept => {
    const broaderConcept = concept.broader ? taxonomy.concepts[concept.broader] : null;
...
export function exportToText(taxonomy: Taxonomy): string {
  ...
  lines.push(`Description: ${taxonomy.metadata.description || 'No description'}`);
  ...
  if (topConcepts.length === 0) {
    ...
    const rootConcepts = Object.values(taxonomy.concepts)
      .filter(c => !c.broader || !taxonomy.concepts[c.broader]);
  ...
  const renderConcept = (concept: TaxonomyConcept, indent: number = 0): void => {
    ...
    if (concept.altLabel && concept.altLabel.length > 0) {
      lines.push(`${prefix}  Alternative labels: ${concept.altLabel.join(', ')}`);
    }
...
export function exportToGraphML(taxonomy: Taxonomy): string {
  ...
  concepts.forEach(concept => {
    ...
    lines.push(`      <data key="label">${escapeXML(concept.prefLabel)}</data>`);
    if (concept.definition) {
      lines.push(`      <data key="definition">${escapeXML(concept.definition)}</data>`);
    }
    lines.push('    </node>');
  });
  ...
  concepts.forEach(concept => {
    ...
    if (concept.broader && taxonomy.concepts[concept.broader]) {
      lines.push(`    <edge source="${concept.id}" target="${concept.broader}">`);
    ...
    if (concept.related) {
      concept.related.forEach(relatedId => {
        if (taxonomy.concepts[relatedId]) {
          lines.push(`    <edge source="${concept.id}" target="${relatedId}">`);
...
export function exportToDOT(taxonomy: Taxonomy): string {
  ...
  lines.push('  node [shape=box, style="rounded,filled", fillcolor=lightblue];');
  ...
  concepts.forEach(concept => {
    ...
    lines.push(`  "${concept.id}" [label="${label}", shape=${shape}, fillcolor=${fillColor}];`);
  });
  ...
  concepts.forEach(concept => {
    ...
    if (concept.broader && taxonomy.concepts[concept.broader]) {
      lines.push(`  "${concept.broader}" -> "${concept.id}" [label="narrower", color=blue];`);
    }
    ...
    if (concept.related) {
      concept.related.forEach(relatedId => {
        if (taxonomy.concepts[relatedId] && concept.id < relatedId) {
          ...
          lines.push(`  "${concept.id}" -> "${relatedId}" [label="related", color=red, dir=both];`);
        }
      });
    }
  });
 ...
 */
export function exportTaxonomy(taxonomy: Taxonomy, format: ExportFormat): string {
  switch (format) {
    case 'csv':
