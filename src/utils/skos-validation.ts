...
export class SKOSValidator {
  ...
  private validateScheme(taxonomy: Taxonomy, errors: ValidationError[], warnings: ValidationError[], info: ValidationError[]) {
  ...
  private validateConcepts(taxonomy: Taxonomy, errors: ValidationError[], warnings: ValidationError[], info: ValidationError[]) {
  ...
  private validateRelationships(taxonomy: Taxonomy, errors: ValidationError[], warnings: ValidationError[], info: ValidationError[]) {
    const concepts = taxonomy.concepts;
  ...
  private validateHierarchy(taxonomy: Taxonomy, errors: ValidationError[], warnings: ValidationError[], info: ValidationError[]) {
    const concepts = taxonomy.concepts;
    ...
    const explicitTopConcepts = new Set(Object.values(concepts).filter(c => c.topConcept).map(c => c.id));
    
    Object.values(concepts).forEach(concept => {
      const hasNoBroader = !concept.broader;
      const isNotTopConcept = !topConceptIds.has(concept.id) && !explicitTopConcepts.has(concept.id);
    ...
    const disconnected = new Set([...allConceptIds].filter(id => !connectedConcepts.has(id)));
...
export function detectSKOSFormat(content: string): 'rdf' | 'turtle' | 'unknown' {
  const trimmed = content.trim();
  
  // Check for XML declaration or RDF root element
  if (trimmed.startsWith('<?xml') || trimmed.includes('<rdf:RDF') || trimmed.includes('<RDF')) {
    return 'rdf';
  }
  
  // Check for Turtle prefixes
  if (trimmed.includes('@prefix') || trimmed.includes('@base') || /^\s*<[^>]+>\s+a\s+/.test(trimmed)) {
    return 'turtle';
  }
...
export function isSKOSContent(content: string): boolean {
  ...
  return lowercased.includes('skos:') || 
