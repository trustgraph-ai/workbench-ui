...
const sampleTaxonomy: Taxonomy = {
  ...
  concepts: {
    ...
    'concept-2': {
      id: 'concept-2',
      prefLabel: 'Plants',
      definition: 'Living organisms that typically produce their own food through photosynthesis',
      narrower: ['concept-4'],
      topConcept: true,
    },
    'concept-3': {
...
describe('SKOS Serializer', () => {
  test('should serialize taxonomy to RDF/XML', () => {
    ...
    // Check scheme
    expect(rdf).toContain('<skos:ConceptScheme rdf:about="http://example.org/test/scheme">');
    expect(rdf).toContain('<skos:prefLabel xml:lang="en">Test Taxonomy</skos:prefLabel>');
    expect(rdf).toContain('<skos:hasTopConcept rdf:resource="http://example.org/test/concept-1"');
    
    // Check concepts
    expect(rdf).toContain('<skos:Concept rdf:about="http://example.org/test/concept-1">');
    expect(rdf).toContain('<skos:prefLabel xml:lang="en">Animals</skos:prefLabel>');
    expect(rdf).toContain('<skos:definition xml:lang="en">Living organisms that feed on organic matter</skos:definition>');
    expect(rdf).toContain('<skos:narrower rdf:resource="http://example.org/test/concept-3"');
    
    // Check concept with alternative labels and examples
    expect(rdf).toContain('<skos:altLabel xml:lang="en">Mammalia</skos:altLabel>');
    expect(rdf).toContain('<skos:example xml:lang="en">Dog</skos:example>');
    expect(rdf).toContain('<skos:notation>T001</skos:notation>');
    expect(rdf).toContain('<skos:scopeNote xml:lang="en">Includes both deciduous and evergreen trees</skos:scopeNote>');
  });
  ...
  test('should serialize taxonomy to Turtle', () => {
    ...
    expect(turtle).toContain('skos:definition "Living organisms that feed on organic matter"@en');
  });
  ...
  test('should handle XML escaping correctly', () => {
    ...
    expect(rdf).toContain('Definition with &quot;quotes&quot; and &apos;apostrophes&apos;');
  });
});
...
describe('SKOS Parser', () => {
  test('should parse simple RDF/XML', async () => {
    ...
    expect(taxonomy.concepts['concept1'].definition).toBe('A test concept definition');
    expect(taxonomy.concepts['concept1'].altLabel).toContain('Alternative Label');
  ...
  test('should handle parsing errors gracefully', async () => {
    ...
    const parser = new SKOSParser();
    await expect(parser.parseRDF(invalidXML, 'test')).rejects.toThrow('Invalid XML format');
  });
});
...
describe('SKOS Validation', () => {
  ...
  test('should detect missing preferred labels', () => {
    ...
    expect(result.errors.some(e => e.code === 'CONCEPT_NO_PREFLABEL')).toBe(true);
  });
  ...
  test('should detect circular references', () => {
    ...
    expect(result.isValid).toBe(false);
    expect(result.errors.some(e => e.code === 'HIERARCHY_CIRCULAR_REFERENCE')).toBe(true);
  });
  ...
  test('should detect invalid concept references', () => {
    ...
    expect(result.isValid).toBe(false);
    expect(result.errors.some(e => e.code === 'CONCEPT_INVALID_BROADER')).toBe(true);
  });
});
